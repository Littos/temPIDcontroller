...
class Settings():
	...
	def __init__(self, settingsfilename="/usr/share/nginx/www/data_to_cnt",mode="off", SP=0, OP=0, P=-0.1, I=1, D=0, interval=1000):
		self.settinsfilename = settingsfilename
		self.mode = mode
		self.SP = SP
		self.OP = OP
		self.P = P
	...
	def Read(self):
		try:
			...
			print("Settings read failure!", 'Error: {}. {}, line: {}'.format(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2].tb_lineno))
			raise
...
class Sequence():
	...
	def Read(self):
		try:
			...
			print("Sequence read failure! ", 'Error: {}. {}, line: {}'.format(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2].tb_lineno))
			raise
	...
	def Execute(self, SP, MV):
		...
		try:
			self.SP = self.SP + self.interval/1000/60*(float(sequence_command[2])-SP)/(float(sequence_command[3])-self.seq_timer)
...
def ExecuteSequence(SP, MV):
	...
	sequencelines = [row.strip('\n') for row in sequence]
	#print("1seq_timer", seq_timer)	
	""""""Check if new manual step input is present""""""
	if(sequencelines[0] != seq_step_input):
		print("New manuals sequence step input")
		seq_step_input = sequencelines[0]
		seq_step = seq_step_input
		seq_timer = 0.0
	...
	#Now break up the sequence command line to command and input values and do the work
	sequence_command = sequencelines[int(seq_step)].split(' ')
	print("sequence_command", sequence_command)
	#If the row doesn't contain anysequence command, set it to "" anyway, so we dont get an error later
	if len(sequence_command) < 2:
		sequence_command = ["-", ""]
	...
	if (sequence_command[1] == 'ramp-to-in'):
		...
		#SPnew = SP + interval*(SPtarget-SP)/(ramptime-seq_timer)
		try:
			SP = SP + interval/1000/60*(float(sequence_command[2])-SP)/(float(sequence_command[3])-seq_timer)
		except ZeroDivisionError:
			SP = float(sequence_command[2])
		
		if(seq_timer >= float(sequence_command[3])):
			#We have reached our time limit so go on with the next step!
			print("Reset seq_timer")
			seq_timer = 0.0
			seq_step = int(seq_step)+1
		
		seq_timer = seq_timer + interval/1000/60
		return SP
		...
		print("hold-for")
		if(seq_timer >= float(sequence_command[2])):
			#We have reached our time limit so go on with the next step!
			print("Reset seq_timer")
			seq_timer = 0.0
			seq_step = int(seq_step)+1		
		...
		print("next-step")
		seq_step = int(sequence_command[2])
		
		return SP
	
	elif (sequence_command[1] == 'temp-diff-to-external'):
		print("temp-diff-to-external")
		seq_step = int(seq_step)+1
		return SP						
...
"""
def PID_Control(mode,SP,MV,OP,P,I,D,e_last,e_int,interval):
	""""""Calculate new OP based on SP, MV and error terms""""""
	...
	if (mode == 'aut'):
		...
		try:
			if (OP >= 100):
				if (P > 0):
					e_int = min(e_int, e_int + (interval/1000.0)/I*err)
				else:
					e_int = max(e_int, e_int + (interval/1000.0)/I*err)
			elif (OP <= 0):
				if (P > 0):
					e_int = max(e_int, e_int + (interval/1000.0)/I*err)	
				else:
				...
				e_int = e_int + (interval/1000.0)/I*err
				
		except (ZeroDivisionError):
			#This occurs if I=0 above 
			e_int = 0
			
		OP = P*(err + e_int + D/(interval/1000.0)*(err-e_last))
...
class Output():
	...
	def __init__(self, SP=0, MV=0, OP=0, no_data_points=1, mirrorfilename = "/var/tmp/temperatures", outputfilename = "/var/tmp/temperatures_workfile"):
		self.SP = SP
		self.MV = MV
		self.OP = OP
	...
	def WriteOutput(self):	
		...
		no_of_records = self.no_of_datapoints
		...
		try:
			...
			with open(self.outputfilename,"a") as filehandle:
				filehandle.write(datestring + "," + str(round(self.SP,2)) + "," + str(round(self.MV,2)) + "," +str(round(self.OP,2)) + "\n");
...
def WriteOutput(SP,MV,OP,no_of_datapoints):
	...
	try:
		...
		with open(outputfilename,"a") as filehandle:
			filehandle.write(datestring + "," + str(round(SP,2)) + "," + str(round(MV,2)) + "," +str(round(OP,2)) + "\n");
	...
	#Copy the working file to the mirror file so it can be used externally!
	shutil.copy2(outputfilename, mirrorfilename)	
"""		
...
if __name__ == "__main__":
	...
	while 1:
		try:
			...
			print('Execution error! ' , 'Error: {}. {}, line: {}'.format(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2].tb_lineno))
			#Here I  roll back the settings to what was before the new ones were read!
			settings = settings_old
			raise
			
		except Exception:
			print('Something really shitty just happend. CRAP! ' , 'Error: {}. {}, line: {}'.format(sys.exc_info()[0], sys.exc_info()[1], sys.exc_info()[2].tb_lineno))	
